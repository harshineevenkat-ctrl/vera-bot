...
def build_compose_prompt(category: str, merchant: Dict, trigger: Dict, customer=None) -> str:
    ...
    customer_str = json.dumps(customer) if customer else "No customer context"
    ...
    offer_patterns = profile["offer_patterns"]

    lines = [
        "You are Vera, magicpin AI assistant for merchant growth. Compose ONE specific grounded message.",
        "",
        "MERCHANT: " + name + " | Category: " + category + " | Location: " + location + " | Rating: " + str(rating),
        "Performance: " + perf_str,
        "Offers: " + offers_str,
        "History: " + history_str,
        "Trigger: " + trigger_str,
        "Customer: " + customer_str,
        "Tone: " + tone + " | Avoid: " + avoid + " | CTA: " + cta_style + " | Patterns: " + offer_patterns,
        "",
        "Rules: Use REAL numbers from data. One CTA. Under 3 sentences. No fake claims. Pick ONE best signal.",
        "",
...
def compose_message(category: str, merchant: Dict, trigger: Dict, customer=None) -> Dict:
    try:
        ...
        return {
            "message": "190 people near you are searching for Dental Check Up. Should I send them your Rs.299 offer?",
            "cta": "Yes, send now",
            "send_as": "Vera",
            "suppression_key": "fallback_" + mid + "_" + ttype,
...
def metadata():
    return {
        ...
        "description": "Grounded message composer for magicpin merchants"
    }
...
def receive_context(req: ContextRequest):
    ...
    if existing and existing["version"] >= req.version:
        return {"accepted": False, "ack_id": "ack_" + uuid.uuid4().hex[:8], "stored_at": datetime.now(timezone.utc).isoformat()}
    ...
    return {"accepted": True, "ack_id": "ack_" + uuid.uuid4().hex[:8], "stored_at": datetime.now(timezone.utc).isoformat()}
...
def reply(req: ReplyRequest):
    ...
    profile = CATEGORY_PROFILES.get(category, CATEGORY_PROFILES["restaurant"])
    history_lines = [m["role"].upper() + ": " + m["content"] for m in history[-5:]]
    history_str = "\n".join(history_lines)
    merchant_str = json.dumps(merchant.get("identity", {}))
    tone = profile["tone"]
    merchant_reply = req.message

    prompt_lines = [
        "You are Vera, magicpin AI assistant. A merchant just replied to your message.",
        "Conversation so far:",
        history_str,
        "Merchant reply: " + merchant_reply,
        "Merchant context: " + merchant_str,
        "Tone: " + tone,
    ...
    try:
        ...
        result = {
            "message": "Great! Let me set that up right away.",
            "cta": "Confirm to proceed",
            "send_as": "Vera",
            "suppression_key": "reply_fallback_" + req.session_id,
            "rationale": "fallback"
        }
